...
switch($moduleinstance->mode){
	case MOD_FLUENCYBUILDER_MODESTUDENTSTUDENT:
		$mode="studentstudent";
		$caption = get_string('gotocst',MOD_FLUENCYBUILDER_LANG);
	...
	default:
		...
		if(has_capability('mod/fluencybuilder:preview',$modulecontext)){
			$caption = get_string('gotocstasteacher',MOD_FLUENCYBUILDER_LANG);
			echo $renderer->show_teacher_newsessionlink($cm,$caption,$moduleinstance);
			echo '<br />';
			echo '<br />';
			$caption = get_string('gotocstasstudent',MOD_FLUENCYBUILDER_LANG);
			echo $renderer->show_student_newsessionlink($cm,$caption,$moduleinstance);
		}else{
